...
public class NewGame extends JFrame {
	...
	private void Two_PlayersActionPerformed(ActionEvent e) {
		...
		for (int i = 0; i <= 1; i++) {
			greenList = GameEngine.playerObjList.get(i).pCards.get("Green");
			brownList = GameEngine.playerObjList.get(i).pCards.get("Brown");
			String greenCards = "";
			String brownCards = "";
			if (greenList != null)
				greenCards = greenList.toString();
			...
			Players_Info
					.setValueAt(GameEngine.playerObjList.get(i).color, i, 0);
			Players_Info
					.setValueAt(GameEngine.playerObjList.get(i).color, i, 1);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(i).personality, i, 2);
			Players_Info.setValueAt(GameEngine.playerObjList.get(i).minionHold,
					i, 3);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(i).buildingHold, i, 4);
			Players_Info.setValueAt(GameEngine.playerObjList.get(i).cashHold,
					i, 5);
			Players_Info.setValueAt("G: " + greenCards + " B: " + brownCards,
					i, 6);
		}
		setRegionInfo();
		Bank_Info.setValueAt(GameEngine.BankHold, 0, 0);
	...
	private void Four_PlayersActionPerformed(ActionEvent e) {
		...
		int turnIndex = 0;
		for (int i = 0; i <= 3; i++) {
			greenList = GameEngine.playerObjList.get(turnIndex).pCards
					.get("Green");
			brownList = GameEngine.playerObjList.get(turnIndex).pCards
					.get("Brown");
			String greenCards = "";
			String brownCards = "";
			if (greenList != null)
			...
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(turnIndex).color, i, 0);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(turnIndex).color, i, 1);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(turnIndex).personality, i, 2);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(turnIndex).minionHold, i, 3);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(turnIndex).buildingHold, i, 4);
			Players_Info.setValueAt(
					GameEngine.playerObjList.get(turnIndex).cashHold, i, 5);
			Players_Info.setValueAt("G: " + greenCards + " B: " + brownCards,
					i, 6);
			turn++;
			turnIndex = turn % (GameEngine.playerObjList.size());
		}
		setRegionInfo();
		Bank_Info.setValueAt(GameEngine.BankHold, 0, 0);
		Two_Players.setEnabled(false);
