 ...
 * <h1>Riots</h1>
 * <p>
 * If there are eight or more Trouble Markers on the board then the game ends
 * immediately
 * </p>
 * 
 * @author mary
	...
	public void executeRandomEvent() {
		...
		if (GameEngine.TMarkerHold <= 4) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int key : GameUtility.playerObjList.get(i).H_Region
						.keySet()) {
					if (GameUtility.playerObjList.get(i).H_Region.get(key).placedbuilding == 1) {
						rNum = GameUtility.playerObjList.get(i).H_Region
								.get(key).regionNumber;
						bCost = bCost
			...
			NewGame.showErrorDialog("Riots Event Occured, Player: " + winnerColor
					+ " wins having total points: " + winner);
			System.exit(0);
		}
		else
		{
			NewGame.showErrorDialog("Riots Event Occured, But is unsuccessful !");			
		}
	}
